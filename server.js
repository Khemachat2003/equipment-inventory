...
app.use((req, res, next) => {
  if (req.url.match(/\.(css|js|png|jpg|jpeg|svg|ico|webp|woff2)$/)) {
    const longCache = /\.(woff2)$/.test(req.url);
    // ฟอนต์: 1 ปี (ไม่เปลี่ยนบ่อย) / อย่างอื่น: 1 วัน
    res.setHeader('Cache-Control', longCache ? 'public, max-age=31536000, immutable' : 'public, max-age=86400');
  }
  next();
});
// ===========================================
